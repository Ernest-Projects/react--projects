...
export const globalStore = createSlice({
    ...
    reducers: {
        ...
        setUpdateSelectedTrack: (state, action:PayloadAction<{id: number, data: Partial<Track>}>) => {
        ...
        setResetAllTracks: (state) => {
            ...
            const reset = (tracks: Track[]) => tracks.map(item => ({...item, playable: false, setted: false}))
