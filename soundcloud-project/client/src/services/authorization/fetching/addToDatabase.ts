...
type ServerResponce = | {ok: true} | {ok: false; reason: "USER_EXISTS"}
 ...
 export const handleAddUserInDatabase = async (inputData:UserInputDataProps ): Promise<ServerResponce> => {
    try {
      const user = {
      ...
      const responce = await fetch("http://localhost:3000/api/users/add/local", {
        method: "POST",
