  ...
  return (
    <>
      <section ref = {fieldRef} style = {{ backgroundImage: "linear-gradient(to bottom, rgb(25,25,25), rgb(45,45,45))", transformStyle: "preserve-3d", transformOrigin: "50% 0%"}}
        className={`flex perspective-[1500px] gap-[.5rem] transform  border  justify-center min-w-[25rem] transition duration-100 [transform:rotateX(15deg)] align-center bg-[rgb(25,25,25)] p-[4rem] opacity-100  relative  grid w-[100%] h-[100%]`}
      >
        {Object.values(cardsArray).slice(0, quantityOfCards).map((el, index) => { 
          return <Card key={index} name={el.id} id = {index} value = {el.value} ></Card>;
        })}
      </section>
    </>
  );
