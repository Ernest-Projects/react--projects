...
export const ClickerButton = ({ onCountClicker, mode, coefficient }: ClickerButtonProps) => {
  ...
  return (
    <>
      ...
      >
        <button
          className={`text-center font-mono absolute place-self-center transition  z-[10] text-3xl ${
        ...
        >
          <p
            className={`${mode === true ? "text-black" : "text-white"} place-self-center z-[-2] group-active:top-[-10rem] opacity-0 duration-300 delay-20 group-active:opacity-100 absolute z-[-1] top-0`}
          >
            +{Number(0.9 + coefficient / coefficient)}
          </p>
        </div>
        <div
          className={`w-[80%] ${
